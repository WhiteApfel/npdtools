...
class NPDTools:
    ...
    **Аргументы**
    ...
    def token(self):
        if (
            self.__token
            and self.__token_lifetime
            and self.__token_lifetime > strftime("%Y-%m-%dT%H:%M:%SZ")
        ):
            return self.__token
        ...
        data = {
            "deviceInfo": {
                ...
                "sourceType": "WEB",
                "metaDetails": {"userAgent": self.__user_agent},
            },
            "refreshToken": self.__refresh_token,
        }
        response = self.client.post(
            f"{self.__api}/auth/token",
            json=data,
            headers=self.__headers("sales", False),
        )
        if (
    ...
    def __auth(self):
        ...
        response = self.client.post(
            f"{self.__api}/auth/lkfl", headers=self.__headers(token=False), json=data
        )
        if (
            response.status_code == codes.OK
            and "json" in response.headers["content-type"]
    ...
    def datestr_valid(self, date: str):
        r = (
            r"([0-9]{4})\-([0-1][0-9])\-([0-3][0-9])T([0-2][0-9])\:([0-5][0-9])\:([0-5][0-9])(\.[0-9]+)?"
            r"([\+\-])([0-1][0-9]):([013][05])"
        )
    ...
    def add_income(
        self,
        services: Union[Services, Service, list, tuple],
        client: Union[Client, list, tuple] = None,
        ...
        if type(services) is Service:
            ...
            type(services) in [tuple, list]
            and len(services) == 3
            and type(services[0]) is str
            and type(services[1]) in [int, float]
            and type(services[2]) is int
        ):
            services = Services(services)
        ...
        response = self.client.post(
            f"{self.__api}/income", json=data, headers=self.__headers()
        )
        if (
            response.status_code == codes.OK
            and "json" in response.headers["content-type"]
    ...
    def cancel_income(
        ...
        response = self.client.post(
            f"{self.__api}/cancel", json=data, headers=self.__headers()
        )
        if (
            response.status_code == codes.OK
            and "json" in response.headers["content-type"]
    ...
    def incomes(
        ...
        if not start or not end:
            ...
            start = (
                (datetime.now() - delta).replace(microsecond=0).astimezone().isoformat()
            )
        elif (
            start
            and (
                (type(start) is str and self.datestr_valid(start))
                or type(start) is datetime
            )
        ) and (
            end
            and (
                (type(end) is str and self.datestr_valid(end)) or type(end) is datetime
            )
        ):
        ...
        response = self.client.get(
            f"{self.__api}/incomes", params=params, headers=self.__headers()
        )
        if (
            response.status_code == codes.OK
            and "json" in response.headers["content-type"]
